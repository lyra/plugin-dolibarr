...
/**
 *  Description and activation class for payzen module.
 */
class modPayzen extends DolibarrModules
{
    private static $MODULE_NUMBER_DOLIBARR = 230050;
    ...
    private static $PROCESS_URL = '/payzen/public/process.php';
    ...
    public function __construct($db)
    {
        ...
        $this->dirs = array("/payzen/temp");
        $this->config_page_url = array("payzen.php@payzen");
        ...
        $this->langfiles = array("payzen@payzen");
        ...
        // Constants.
        $this->const = array(
            1 => array('PAYZEN_RETURN_MODE', 'chaine', self::$RETURN_MODE, '', 1),
            2 => array('PAYZEN_REDIRECT_SUCCESS_TIMEOUT', 'chaine', self::$REDIRECT_SUCCESS_TIMEOUT, '', 1),
            3 => array('PAYZEN_REDIRECT_SUCCESS_MESSAGE', 'chaine', $langs->trans('PAYZEN_REDIRECT_MESSAGE_TEXT'), '', 1),
            4 => array('PAYZEN_REDIRECT_ERROR_TIMEOUT', 'chaine', self::$REDIRECT_ERROR_TIMEOUT, '', 1),
            5 => array('PAYZEN_REDIRECT_ERROR_MESSAGE', 'chaine', $langs->trans('PAYZEN_REDIRECT_MESSAGE_TEXT'), '', 1),
            6 => array('PAYZEN_VALIDATION_MODE', 'chaine', self::$VALIDATION_MODE, '', 1),
            7 => array('PAYZEN_LANGUAGE', 'chaine', self::$LANGUAGE, '', 1),
            8 => array('PAYZEN_URL_CHECK', 'chaine', dol_buildpath(self::$PROCESS_URL, 2), '', 1),
            9 => array('PAYZEN_URL_RETURN', 'chaine', dol_buildpath(self::$PROCESS_URL, 2), '', 1),
            10 => array('PAYZEN_CTX_MODE', 'chaine', self::$CTX_MODE, '', 1),
            12 => array('PAYZEN_REST_API_URL_CHECK', 'chaine', dol_buildpath(self::$PROCESS_URL, 2), '', 1),
            13 => array('PAYZEN_GATEWAY_URL', 'chaine', self::$GATEWAY_URL, '', 1),
            14 => array('PAYZEN_REST_API_URL', 'chaine', self::$REST_URL, '', 1),
            15 => array('PAYZEN_REST_API_CLIENT_STATIC_URL', 'chaine', self::$STATIC_URL, '', 1),
            16 => array('PAYZEN_SITE_ID', 'chaine', self::$SITE_ID, '', 1),
            17 => array('PAYZEN_KEY_TEST', 'chaine', self::$KEY_TEST, '', 1),
            18 => array('PAYZEN_KEY_PROD', 'chaine', self::$KEY_PROD, '', 1),
            19 => array('PAYZEN_CARD_INFO_MODE', 'chaine', self::$FORM_TYPE, '', 1),
            20 => array('PAYZEN_DEVELOPPED_BY', 'chaine', "Lyra Network (https://www.lyra.com/)", '', 1),
            21 => array('PAYZEN_CONTACT_EMAIL', 'chaine', self::$SUPPORT_EMAIL, '', 1),
            22 => array('PAYZEN_MODULE_VERSION', 'chaine', self::$PLUGIN_VERSION, '', 1),
            23 => array('PAYZEN_GATEWAY_VERSION', 'chaine', self::$GATEWAY_VERSION, '', 1)
        );

        if (! isset($conf->payzen) || ! isset($conf->payzen->enabled)) {
            $conf->payzen = new stdClass();
            $conf->payzen->enabled = 0;
        }
        ...
        // Boxes/Widgets: list of php file(s) stored in payzen/core/boxes that contains a class to show a widget.
        $this->boxes = array();
        ...
        $this->rights[$r][1] = 'Read objects of payzen'; // Permission label.
        $this->rights[$r][4] = 'myobject';
        $this->rights[$r][5] = 'read';
        $r++;
        $this->rights[$r][0] = $this->numero + $r;
        $this->rights[$r][1] = 'Create/Update objects of payzen';
        $this->rights[$r][4] = 'myobject';
        $this->rights[$r][5] = 'write';
        $r++;
        $this->rights[$r][0] = $this->numero + $r;
        $this->rights[$r][1] = 'Delete objects of payzen';
        $this->rights[$r][4] = 'myobject';
    ...
    {
        $result = $this->_load_tables('/payzen/sql/');
