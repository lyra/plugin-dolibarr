...
dol_include_once('/payzen/class/PayzenResponse.php');
require_once DOL_DOCUMENT_ROOT . "/core/lib/admin.lib.php";

global $conf, $notification_type;

$source = '';
$payzen_response = null;
...
if (GETPOSTISSET('signature')) {
    ...
    $payzen_response = new PayzenResponse(
        $data,
        $conf->global->PAYZEN_CTX_MODE,
        $conf->global->PAYZEN_KEY_TEST,
        $conf->global->PAYZEN_KEY_PROD,
        modPayzen::getDefault('SIGN_ALGO')
    );

    // Check the authenticity of the request.
    if (! $payzen_response->isAuthentified()) {
        payzen_syslog("Invalid signature with data: " . print_r($data, true), '', LOG_ERR);
    ...
    $key = ($notification_type === 'IPN') ? getPayzenPrivateKey() : getPayzenReturnKey();

    // Check the authenticity of the request.
    if (! modPayzen::checkHash($_POST, $key)) {
        payzen_syslog("Invalid signature with data: " . print_r($_POST, true), '', LOG_ERR);
        print_notification_message($data, $source, 'INVALID_SIGNATURE', ERROR_INVALID_SIGNATURE);
    }

    $answer = json_decode($data['kr-answer'], true);
    if (is_array($answer)) {
        $data = modPayzen::convertRestResult($answer);
        $payzen_response = new PayzenResponse($data, null, null, null);
    }
} else {
...
if ($notification_type !== 'IPN') {
    llxHeaderPayzen($langs->trans("PAYZEN_PAYMENT_FORM"));
}

$amount = $payzen_response->get('amount');
$currency = PayzenApi::findCurrencyByNumCode($payzen_response->get('currency'));

$order_id = $payzen_response->get('order_id');
$full_tag = $payzen_response->getExtInfo('full_tag');
$trans_status = $payzen_response->get('trans_status');

payzen_syslog("----------- NOTIFICATION TYPE = " . $notification_type . " -----------", $order_id, LOG_INFO);

// Get payment source (INVOICE, ORDER, FREE, DONATION...)
$source = getSourceType($full_tag);
payzen_syslog('source=' . $source . ' || Amount=' . $amount . ' || Order_id=' . $order_id . ' || status=' . $trans_status, $order_id);
...
if ($payzen_response->isAcceptedPayment()) {
    ...
    if ($source === 'free') {
        $ext_payment_id = $payzen_response->get('trans_uuid');
    }
    ...
    if (! $alreadyPaid) { // If no record of the payment exists.
        if ($source === 'invoice' || $source === 'free' ) { // If it is a facture or a free amount.
            ...
            if ($result || $source === 'free') {
                $now = dol_now();
                $ipaddress = $payzen_response->get('ip_address');
                $FinalPaymentAmt = round($currency->convertAmountToFloat($amount), $currency->getDecimals());
                $transaction_uuid = $payzen_response->get('trans_uuid');
                $mode = $conf->global->PAYZEN_CTX_MODE;
                $service = (($mode === 'TEST') ? 'Payzen Test' : 'Payzen Production');
                ...
                $paymentTypeId = $conf->global->PAYZEN_PAYMENT_MODE_FOR_PAYMENTS;
                ...
                if (! $error) {
                    if ($source === 'free') { // Only create record on llx_paiement for free amount payment.
                        ...
                        if ($resql) {
                            ...
                            payzen_syslog("Record Payment Created", $order_id);
                        }
                    } else {
                    ...
                    if ($paiement_id < 0) {
                        payzen_syslog($paiement->error . ' ' . join("<br>\n", $paiement->errors), $order_id, LOG_ERR);
                        $error++;
                    } else {
                        payzen_syslog("Record Payment Created", $order_id);
                    }
                ...
                if (! $error && ! empty($conf->banque->enabled) && ! empty($conf->global->PAYZEN_DOLIBARR_BANK_ACCOUNT)) {
                    $bankaccountid = $conf->global->PAYZEN_DOLIBARR_BANK_ACCOUNT;
                    if ($bankaccountid > 0) {
                        ...
                        payzen_syslog(NOT_BANK_PAYMENT, $order_id, LOG_WARNING);
                    }
                }

                if (! $error) {
                    $db->query("COMMIT");
                    payzen_syslog('COMMIT', $order_id);
                } else {
                    $db->rollback();
                    payzen_syslog('ROLLBACK', $order_id, LOG_ERR);
                }
            }
        }
...
} elseif ($payzen_response->isCancelledPayment()) {
...
function print_result_payment($data, $source, $resultType, $additionalMessages = array())
{
    ...
    if ($resultType === 'ERROR') {
        ...
        if ($status == 'REFUSED') {
            $message = $langs->trans("PAYZEN_REFUSED_PAYMENT_TEXT");
            $messageDesc = $langs->trans("PAYZEN_REFUSED_PAYMENT_MSG");
        } elseif ($status == 'CANCELLED') {
            $message = $langs->trans("PAYZEN_CANCELLED_PAYMENT_TEXT");
            $messageDesc = $langs->trans("PAYZEN_CANCELLED_PAYMENT_MSG");
        } elseif ($status == 'ABANDONED') {
            header("Location: " . $url);
        } else {
            $message = $langs->trans("PAYZEN_ERROR_PAYMENT_TEXT");
            $messageDesc = $langs->trans("PAYZEN_ERROR_PAYMENT_MSG");
        }

        $messageClic = $langs->trans("PAYZEN_TRY_AGAIN");
        ...
        $message = $langs->trans("PAYZEN_THANK_FOR_PAYMENT_TEXT");
        $messageDesc = $langs->trans("PAYZEN_SUCCESS_PAYMENT_TEXT");
        $messageClic = $langs->trans("PAYZEN_DETAILS");
    } elseif ($resultType === 'PENDING') {
        $url = getOnlinePaymentUrl(0, $source, $order_id);
        $message = $langs->trans("PAYZEN_PENDING_PAYMENT");
        $messageDesc = $langs->trans("PAYZEN_PENDING_PAYMENT_DESC");
        $messageClic = $langs->trans("PAYZEN_DETAILS");
        if ($source === 'free') {
            $print_button = false;
        }
    } elseif ($resultType === 'INVALID_SIGNATURE') {
        $message = $langs->trans("PAYZEN_INVALID_SIGNATURE");
        $messageDesc = $langs->trans("PAYZEN_INVALID_SIGNATURE_DESC");
        $print_button = false;
    } elseif ($resultType === 'INVALID_AMOUNT') {
         $message = $langs->trans("PAYZEN_INVALID_AMOUNT");
         $messageDesc = $langs->trans("PAYZEN_INVALID_AMOUNT_DESC");
         $print_button = false;
    }
...
function print_summary_payment_details($data, $source)
{
    ...
    if ($source == 'free') {
        // Creditor.
        print '<tr class="CTableRow' . ($var ? '1' : '2') . '"><td class="CTableRow' . ($var ? '1' : '2') . '">' . $langs->trans("PAYZEN_CREDITOR");
        print '</td><td class="CTableRow' . ($var ? '1' : '2') . '"><b>' . $creditor . '</b>';
        print '</td></tr>' . "\n";

        // Amount.
        print '<tr class="CTableRow' . ($var ? '1' : '2') . '"><td class="CTableRow' . ($var ? '1' : '2') . '">' . $langs->trans("PAYZEN_AMOUNT");
        if (empty($amount)) {
            print ' (' . $langs->trans("PAYZEN_TO_COMPLETE") . ')';
        }
        ...
        // Tag.
        print '<tr class="CTableRow' . ($var ? '1' : '2') . '"><td class="CTableRow' . ($var ? '1' : '2') . '">' . $langs->trans("PAYZEN_PAYMENT_CODE");
    ...
    if ($source == 'order') {
        ...
        // Debitor.
        print '<tr class="CTableRow' . ($var ? '1' : '2') . '"><td class="CTableRow' . ($var ? '1' : '2') . '">' . $langs->trans("PAYZEN_THIRD_PARTY");
        print '</td><td class="CTableRow' . ($var ? '1' : '2') . '"><b>' . $order->thirdparty->name . '</b>';
        print '</td></tr>' . "\n";

        // Object.
        $text = '<b>' . $langs->trans("PAYZEN_PAYMENT_ORDER_REF", $order->ref) . '</b>';
        if (GETPOST('desc', 'alpha')) {
            $text = '<b>' . $langs->trans(GETPOST('desc', 'alpha')) . '</b>';
        }

        print '<tr class="CTableRow' . ($var ? '1' : '2') . '"><td class="CTableRow' . ($var ? '1' : '2') . '">' . $langs->trans("PAYZEN_DESIGNATION");
...
function verifyAmount($source, $data)
{
    ...
    $order_id = $data['vads_order_id'];
    $currency = PayzenApi::findCurrencyByNumCode($data['vads_currency']);
    $amount = round($currency->convertAmountToFloat($data['vads_amount']), $currency->getDecimals());
    ...
    if ($source === 'invoice') {
        ...
        if ($factureAmount !== (float) $amount) {
            ...
            if ($notification_type !== 'IPN') {
                $additionalInfo = array($langs->trans("PAYZEN_IPN_AMOUNT", $amount), $langs->trans("PAYZEN_INVOICE_AMOUNT", strval($factureAmount)));
            }
...
function setStatusNotePrivate($invoice, $trans_status, $order_id)
{
    ...
    if (! $error) {
        $db->commit();
        payzen_syslog("Set Private Note = " . $trans_status, $order_id);
    } else {
        $db->rollback();
        payzen_syslog("Not Set Private Note = " . $trans_status, $order_id);
    }
}
...
function print_notification_message($data, $source, $messageType, $supportMessage, $additionalInfo = array(), $verify_context = 0)
{
    ...
    $ctx_mode = $conf->global->PAYZEN_CTX_MODE;
    if ($verify_context == 0) {
        $ctx_mode = 'PRODUCTION';
    }
    ...
    payzen_syslog($supportMessage, $data['vads_order_id'], $log);
    if ($notification_type !== 'IPN') {
